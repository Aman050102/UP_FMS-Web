    ...
    <div className="min-h-screen bg-bg-main font-kanit p-5 md:p-15 animate-in fade-in duration-500">
      <main className="max-w-[1100px] mx-auto">
        ...
        <div className="text-center mb-8 mt-5">
          ...
          <h1 className="text-3xl md:text-4xl font-extrabold text-[#0f172a] mb-2">บันทึกการใช้สนาม</h1>
        </div>

        {!currentFacility ? (
          <section className="bg-surface rounded-3xl shadow-sm border border-border-main p-6 md:p-10">
            <h3 className="text-xl font-bold mb-6 text-center text-text-main">เลือกประเภทสนาม</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {TOP.map((f) => (
                ...
                >
                  ...
                  <span className="font-bold text-lg text-slate-700">{f.name}</span>
                </button>
              ))}
            </div>
          </section>
          ...
          <section className="bg-surface rounded-3xl shadow-sm border border-primary/10 p-6 md:p-10 animate-in slide-in-from-bottom-4 duration-300">
            ...
            {currentFacility === "outdoor" && !selectedSub ? (
              <div className="animate-in fade-in duration-300">
                <h3 className="text-xl font-bold mb-6 text-center">ระบุสนามกลางแจ้งย่อย</h3>
                  ...
                  {OUTDOOR_SUBS.map((s) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={isDone}
                      >
                        {s.name} {isDone && <span className="text-green-500">✔</span>}
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="max-w-[600px] mx-auto animate-in zoom-in-95 duration-200">
                <div className="text-center mb-8">
                  <h2 className="text-2xl font-bold text-text-main">
                    ...
                    {selectedSub && <span className="text-primary"> / {selectedSub.name}</span>}
                  </h2>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      type="number" placeholder="0" value={students}
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      className="w-full p-4.5 border-2 border-slate-200 rounded-2xl text-2xl font-bold text-center outline-none focus:border-primary focus:bg-primary-soft/10 transition-all"
                      type="number" placeholder="0" value={staff}
                      onChange={(e) => setStaff(e.target.value)}
                    />
                  </div>
                </div>

                {error && <div className="p-3 bg-red-50 text-red-500 rounded-lg text-center font-bold mb-4 animate-bounce">{error}</div>}
      ...
      {/* Success Overlay */}
      <div id="overlay" className="fixed inset-0 bg-slate-900/85 backdrop-blur-md hidden items-center justify-center z-[1000] p-5">
        <div className="bg-white p-12 rounded-[32px] text-center w-full max-w-[420px] shadow-2xl animate-in zoom-in-50 duration-300">
          ...
          <h2 className="text-3xl font-extrabold text-slate-800 mb-2">บันทึกเรียบร้อย</h2>
          <p className="text-slate-500">ข้อมูลผู้เข้าใช้งานสนามถูกจัดเก็บในระบบแล้ว</p>
        </div>
      </div>
    </div>
  );
