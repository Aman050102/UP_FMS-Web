    ...
    <div className="min-h-screen bg-bg-main font-kanit p-4 md:p-8 animate-in fade-in duration-500">
      <div className="max-w-[1200px] mx-auto space-y-6">
        <header className="bg-surface p-6 rounded-3xl border border-border-main shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
          <div>
            ...
            <p className="text-text-muted text-sm">
              ตรวจสอบความว่างและออกเอกสารขออนุมัติ (13pt)
            </p>
          </div>
          <button
        ...
        <section className="bg-surface rounded-3xl border border-border-main shadow-sm overflow-hidden">
          <div className="p-4 border-b bg-gray-50 font-bold text-text-main">
            รายการจองล่าสุด
          </div>
                ...
                {bookings.map((b: any) => (
                  <tr key={b.id} className="hover:bg-gray-50">
                    <td className="p-4 font-bold">
                      {b.requesterName} <br />
                      <small className="font-normal">{b.department}</small>
                    </td>
                    ...
                    <td className="p-4">
                      <div className="flex justify-center gap-2">
                        <a
                          href={`${API_BASE}/api/staff/booking/pdf/${b.id}`}
                          target="_blank"
                        ...
                        <button
                          onClick={() =>
                            window.open(
                              `${API_BASE}/api/staff/booking/pdf/${b.id}`,
                              "_blank",
                            )
                          }
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
          <form
            ...
            className="bg-white w-full max-w-2xl rounded-[32px] p-8 shadow-2xl animate-in zoom-in-95 max-h-[90vh] overflow-y-auto"
          >
            <div className="flex justify-between items-center mb-6 border-b pb-4">
              <h2 className="text-xl font-black text-text-main">
                ระบุรายละเอียดการจองตามฟอร์ม
              </h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <label className="text-xs font-bold text-text-muted">
                  ชื่อ-นามสกุล ผู้ขอใช้สนาม [cite: 59]
                </label>
                <input
                  name="requesterName"
                  required
                  className="w-full p-3 bg-bg-main border rounded-xl"
                  placeholder="เช่น นายอามาน อาลีแก"
                />
              </div>
              ...
              <div>
                [cite_start]
                <label className="text-xs font-bold text-text-muted">
                  วัตถุประสงค์เพื่อ [cite: 63]
                </label>
                <textarea
                  name="purposeDetail"
                  className="w-full p-3 bg-bg-main border rounded-xl h-20"
                  placeholder="เช่น จัดกิจกรรมโครงการสานสายใย..."
                ></textarea>
              </div>
            </div>
            ...
            <div className="flex gap-4 pt-6 border-t mt-6">
              <button
                type="submit"
                className="flex-1 bg-primary text-white py-4 rounded-2xl font-black shadow-lg"
              >
                บันทึกและสร้างแบบฟอร์ม PDF
              </button>
              <button
                type="button"
                onClick={() => setShowModal(false)}
                className="px-8 bg-gray-100 rounded-2xl font-bold"
              >
                ยกเลิก
              </button>
            </div>
