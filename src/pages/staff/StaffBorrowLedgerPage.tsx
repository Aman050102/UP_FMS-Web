...
export default function StaffBorrowReportPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const historyRes = await fetch(
        `${API}/api/equipment/history-range?${params}`,
      );
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((r) =>
      (r.student_id || "").toLowerCase().includes(studentSearch.toLowerCase()),
    );
    ...
    <main className="min-h-screen bg-[#F8FAFC] font-kanit p-4 md:p-8 space-y-6">
      <section className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
        <div className="flex flex-wrap gap-4 items-end">
          <div className="flex-1 min-w-[300px] grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1">
                <Calendar size={12} /> เริ่มต้น
              </span>
              <input
                type="date"
                className="w-full p-2 bg-slate-50 border rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500/20"
                value={dateFrom}
          ...
          <button
            onClick={fetchData}
            className="px-6 py-2 bg-indigo-600 text-white rounded-lg font-bold text-sm flex items-center gap-2 hover:bg-indigo-700 transition-all"
          >
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex bg-slate-100 p-1 rounded-lg">
              <button
                ...
                className={`p-2 rounded ${viewMode === "pie" ? "bg-white shadow-sm text-indigo-600" : "text-slate-400"}`}
              >
                <PieChart size={16} />
              </button>
              <button
                onClick={() => setViewMode("bar")}
                className={`p-2 rounded ${viewMode === "bar" ? "bg-white shadow-sm text-indigo-600" : "text-slate-400"}`}
              >
                <BarChart3 size={16} />
              </button>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-indigo-600 rounded-3xl p-8 text-white shadow-lg shadow-indigo-200">
            ...
            <div className="text-6xl font-black mt-1">
              ...
              <span className="text-lg font-normal opacity-60">ชิ้น</span>
            </div>
          </div>

          <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm overflow-hidden">
            <h4 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
              <Package size={18} className="text-indigo-600" /> ยอดนิยม
            </h4>
              ...
              {filteredRecords.map((r, i) => (
                <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-4 font-bold text-slate-700">
                    {r.student_id}{" "}
                    <small className="block font-normal text-slate-400 text-[10px]">
                      {r.faculty}
                    </small>
                  </td>
