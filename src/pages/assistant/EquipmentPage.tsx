...
export default function EquipmentPage() {
        ...
        if (returnData.ok) {
          ...
          const grouped = returnData.rows.reduce((acc: any, curr: any) => {
            ...
            if (!acc[sId]) {
              acc[sId] = {
                id: sId,
                faculty: curr.faculty || "ไม่ระบุคณะ",
                items: [],
              };
            }
            acc[sId].items.push({
  ...
  const handleUpdateBorrowQty = (itemName: string, delta: number) => {
    ...
    if (exist) {
      ...
      if (!isBackdate && newQty > (stockItem?.stock || 0) && delta > 0)
      ...
      if (!isBackdate && (stockItem?.stock || 0) <= 0)
      ...
      <div className="space-y-5">
        {activeTab === "borrow" && (
          <div className="flex flex-col gap-5 animate-in fade-in duration-300 text-left">
            <section className="bg-white border border-border-main rounded-xl p-5 shadow-sm">
              <div className="flex justify-between items-center mb-4">
                <h4 className="text-primary flex items-center gap-2.5 text-lg font-bold">
                  ...
                  {isBackdate
                    ? "บันทึกสถิติย้อนหลัง (ไม่ตัดสต็อก)"
                    : "ข้อมูลผู้ยืม"}
                </h4>
                <button
                  onClick={() => {
                  ...
                  className={`text-[10px] px-3 py-1.5 rounded-md font-bold transition-all border ${
                    isBackdate
              ...
              {isBackdate ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in duration-300">
                  <input
                    ...
                    className="w-full p-2.5 border border-border-main rounded-lg outline-none"
                  ...
                  <input
                    type="text"
                    placeholder="รหัสนิสิต / รหัสนักเรียน"
                    className="w-full p-2.5 border border-border-main rounded-lg outline-none"
                  ...
                  />
                  <select
                    className="w-full p-2.5 border border-border-main rounded-lg bg-white"
                  ...
                  <input
                    ...
                    maxLength={10}
                    className="w-full p-2.5 border border-border-main rounded-lg"
            ...
            <section className="bg-white border border-border-main rounded-xl p-5 shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {stocks.map((item) => (
                  <div
                    key={item.name}
                    className="flex justify-between items-center p-3 bg-gray-50 border border-border-main rounded-xl"
                  >
                    <div className="flex items-center gap-3 overflow-hidden">
                      ...
                      <div className="overflow-hidden">
                        ...
                        <small
                          className={`text-[10px] font-bold ${item.stock > 0 ? "text-gray-400" : "text-red-500"}`}
                        >
                    ...
                    <div className="flex items-center gap-2 bg-white p-1 px-2 rounded-full border border-border-main shadow-sm">
                      <button
                        className="w-6 h-6 rounded-full flex items-center justify-center hover:bg-primary hover:text-white"
                        onClick={() => handleUpdateBorrowQty(item.name, -1)}
                      >
                        <Minus size={12} />
                      </button>
                      <span className="font-extrabold text-sm min-w-[15px] text-center">
            ...
            {borrowItems.length > 0 && (
              <section
                className={`border-t-4 rounded-xl p-5 animate-in slide-in-from-bottom duration-300 ${isBackdate ? "bg-blue-50 border-blue-500" : "bg-primary/5 border-primary"}`}
              >
                <button
        ...
        {activeTab === "return" && (
          <div className="space-y-3 animate-in fade-in duration-300 text-left">
            {pendingReturns.length === 0 ? (
              <div className="text-center py-20 bg-white border border-dashed border-border-main rounded-xl text-text-muted italic">
                ไม่มีรายการค้างคืน
              </div>
            ) : (
              pendingReturns.map((user) => (
                <div
                  key={user.id}
                  className="border border-border-main rounded-xl bg-white shadow-sm overflow-hidden"
                >
                  <div
        ...
        {activeTab === "history" && (
          <div className="bg-white border border-border-main rounded-xl p-6 shadow-sm overflow-x-auto animate-in fade-in duration-300 text-left">
            <h4 className="text-primary flex items-center gap-2.5 font-bold mb-6">
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="p-3 text-left">ผู้ยืม</th>
                ...
                {borrowHistory.map((h: any, i: number) => (
                  <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="p-3 text-center">
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-bold ${h.action === "return" ? "bg-green-100 text-green-600" : "bg-orange-100 text-orange-600"}`}
                      >
