...
import {
  Bell, LogOut, ChevronDown, Users, Check, Settings, ShieldAlert
} from "lucide-react";
...
export default function MainHeader({ onToggleMenu, role }: any) {
  ...
  const fetchUnreadCount = async () => {
    try {
      ...
      if (data.ok) {
        const unread = data.rows.filter((n: any) => n.is_read === 0 || n.is_read === false).length;
  ...
  const accessibleRoles = isAdmin ? allRoles : allRoles.filter(r => r.id === 'user' || r.id === userRole);
  const isStaffOrAdmin = userRole === "staff" || isAdmin;

  const renderAvatar = (sizeClass: string, textClass: string) => {
    const hasImage = headerInfo.profileImage && headerInfo.profileImage !== "null";
    const initial = headerInfo.displayName.trim().charAt(0).toUpperCase();

    return (
      <div className={`${sizeClass} rounded-full bg-[#5f5aa2] flex items-center justify-center text-white ${textClass} font-bold overflow-hidden border border-white/20 shadow-inner`}>
        {hasImage ? (
          <img src={headerInfo.profileImage!} alt="Profile" className="w-full h-full object-cover" />
        ) : (
          <span>{initial}</span>
        )}
      </div>
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-header bg-[#5f5aa2] flex items-center justify-between px-4 z-50 shadow-md font-kanit">
      <div className="flex items-center gap-4">
        ...
        <img
          src={dsaLogo} alt="Logo" className="h-[45px] cursor-pointer object-contain brightness-0 invert"
      ...
      <div className="flex items-center gap-3">
        {isStaffOrAdmin && (
          <button className="p-2.5 rounded-full text-white hover:bg-white/10 relative cursor-pointer" onClick={() => navigate("/staff/notifications")}>
            <Bell size={22} />
            {unreadCount > 0 && (
        ...
        <div className="relative" ref={profileRef}>
          <button className="flex items-center gap-2 bg-black/20 hover:bg-black/30 p-1.5 pr-3 rounded-xl border border-white/10 group cursor-pointer text-left transition-all" onClick={() => setShowProfile(!showProfile)}>
            {renderAvatar("w-[32px] h-[32px]", "text-xs")}
            <div className="flex flex-col leading-tight hidden sm:block">
              <span className="text-white text-sm font-medium">{headerInfo.displayName}</span>
            </div>
            ...
            <div className="absolute top-[50px] right-0 w-[280px] bg-white rounded-2xl shadow-2xl border border-gray-100 py-2 z-[3000] text-[#1e293b] text-left animate-in fade-in slide-in-from-top-2">
              <div className="px-4 py-3 border-b border-gray-50 mb-1 flex items-center gap-3">
                ...
                <div className="overflow-hidden">
                  <p className="text-[11px] text-indigo-500 font-bold truncate">{headerInfo.userEmail}</p>
                  <p className="font-bold text-sm text-gray-800 truncate">{headerInfo.displayName}</p>
                </div>
              </div>

              <div className="px-2 py-2 border-b border-gray-50">
                <div className="px-3 py-1 flex items-center gap-2 text-gray-400 mb-1"><Users size={14} /><span className="text-[12px] font-bold uppercase">กลุ่มสิทธิ์การใช้งาน</span></div>
                {accessibleRoles.map((r) => (
                  <button key={r.id} onClick={() => { navigate(r.path); setShowProfile(false); }} className={`w-full flex items-center justify-between px-3 py-2 rounded-lg transition-all ${role === r.id ? 'bg-indigo-50 text-indigo-700 font-bold' : 'hover:bg-gray-50 text-gray-600'}`}>
              ...
              <div className="p-1">
                {isAdmin && (
                  <button className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-indigo-50 text-indigo-600 rounded-lg font-bold text-left" onClick={() => { navigate("/admin/manage-access"); setShowProfile(false); }}>
                    <ShieldAlert size={18} /><span className="text-[14px]">จัดการสิทธิ์สมาชิก</span>
                  </button>
                )}
                <button className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-gray-50 text-gray-600 rounded-lg text-left" onClick={() => { navigate("/profile-settings"); setShowProfile(false); }}>
                  <Settings size={18} className="text-gray-400" /><span className="text-[14px]">จัดการโปรไฟล์</span>
                </button>
                <button className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-red-50 text-red-500 rounded-lg font-bold text-left" onClick={() => { localStorage.clear(); window.location.href = "/login"; }}>
