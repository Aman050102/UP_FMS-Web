...
import {
  ...
  MessageSquare, CalendarCheck, CheckCircle, Trophy, User as UserIcon, ShieldCheck
} from "lucide-react";
...
export default function Sidebar({ open, onClose, role }: any) {
  ...
  const menus = {
    user: [
      ...
      { to: "/user/booking", label: "ระบบการจองสนามกีฬา", icon: <CalendarCheck size={20} /> },
    ],
    assistant: [
      ...
      <aside className={`fixed top-0 left-0 h-full w-[280px] bg-white z-[1002] flex flex-col shadow-2xl transition-transform duration-300 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="p-6 border-t mt-auto flex items-center gap-3 text-primary font-bold bg-primary-soft/20">
          ...
          <div className="flex flex-col overflow-hidden leading-tight text-left">
            <span className="text-[10px] uppercase opacity-60 font-black tracking-wider">{role} View</span>
